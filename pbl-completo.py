...
# Variável para as questões difíceis serem apresentadas pós ranking
dificil_maior = 0
nomedif = ""

# Mensagem inicial para o usuário
print('Olá usuário! Bem vindo ao Software de gerenciamento de pontos da Maratona de Programação.')
print('Por favor, preencha corretamente todos os dados.')
print('')

# Menu para o usuário ir para as ações do ranking
# O menu só funciona em sequência, pois as opções 2 e 3 necessitam do preenchimento de dados dos pesos

Menu = 0
while Menu != 4:
  print("Menu de gerenciamento do Sistema:")
  ...
  opcao = input("\nEscolha uma opção para o ranking: ")
  ...
  if opcao == "1":
    ...
    print('')
# Apresentar equipe com maior número de questões difíceis
    if rank1_qd == rank1_qd:
        nomedif = ranking1
        dificil_maior = rank1_qd
    elif rank2_qd > rank1_qd and rank2_qd > rank3_qd and rank2_qd > rank4_qd and rank2_qd > rank5_qd:
        nomedif = ranking2
        dificil_maior = rank2_qd
    elif rank3_qd > rank1_qd and rank3_qd > rank2_qd and rank3_qd > rank4_qd and rank3_qd > rank5_qd:
        nomedif = ranking3
        dificil_maior = rank3_qd
    elif rank4_qd > rank1_qd and rank4_qd > rank2_qd and rank4_qd > rank3_qd and rank4_qd > rank5_qd:
        nomedif = ranking4
        dificil_maior = rank4_qd
    elif rank5_qd > rank1_qd and rank5_qd > rank2_qd and rank5_qd > rank3_qd and rank5_qd > rank4_qd:
        nomedif = ranking5
        dificil_maior = rank5_qd
    ...
    print(f"1º lugar: {ranking1} | Pontuação: {rank1_pon:.2f} | QD: {rank1_qd} | QM: {rank1_qm} | QF: {rank1_qf} | Tempo: {rank1_tem} ")
    print(f"2º lugar: {ranking2} | Pontuação: {rank2_pon:.2f} | QD: {rank2_qd} | QM: {rank2_qm} | QF: {rank2_qf} | Tempo: {rank2_tem} ")
    print(f"3º lugar: {ranking3} | Pontuação: {rank3_pon:.2f} | QD: {rank3_qd} | QM: {rank3_qm} | QF: {rank3_qf} | Tempo: {rank3_tem} ")
    print(f"4º lugar: {ranking4} | Pontuação: {rank4_pon:.2f} | QD: {rank4_qd} | QM: {rank4_qm} | QF: {rank4_qf} | Tempo: {rank4_tem} ")
    print(f"5º lugar: {ranking5} | Pontuação: {rank5_pon:.2f} | QD: {rank5_qd} | QM: {rank5_qm} | QF: {rank5_qf} | Tempo: {rank5_tem} ")
    print('-------------------------------------------------------------------------------------------------')
    ...
    # Equipe que mais acertou questões difíceis
    print(f"A equipe com mais ponto dificeis é {nomedif} com {dificil_maior} acertos")
    print('')
